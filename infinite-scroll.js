    ...
    //Initialization methods
		...
		function initializeProperties(){
		  ...
		  var totalHeight = settings.lodashVariables.dataList.length * settings.rowHeight;
			settings.$parentElem.height(totalHeight);
			settings.$fixedHeightContainerElem[0].scrollTop = 0;
				
			settings.$parentElem.css({position: "relative"});
			settings.parentOffsetTop = settings.$parentElem.offset().top;
			
			var fixedContainerElemHeight = settings.$fixedHeightContainerElem[0].clientHeight;
		...
		function getParentClientRect(){
			settings.parentClientRect = settings.$fixedHeightContainerElem[0].getBoundingClientRect();
			return settings.parentClientRect;
		}
		
		//Removing & Attaching Scroll events.
		function initializeEvents(){
			settings.$fixedHeightContainerElem.off("scroll.paginationGrid");
			settings.$fixedHeightContainerElem.on("scroll.paginationGrid", handleScrollEvent);
		}
		...
		function handleScrollEvent1(evt){
			var scrollTop = settings.$fixedHeightContainerElem[0].scrollTop;
		...
		**/
		...
		function renderPageData(startCount, endCount, isScrollDown, removeAllChild){
			var rowParentElem = settings.$rowParentElem[0];
		...
		this.destroyGrid = function(){
			settings.$fixedHeightContainerElem && settings.$fixedHeightContainerElem.off("scroll.paginationGrid");
		}
	}
